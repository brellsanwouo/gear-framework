...
DB_HOST = os.environ.get("DB_HOST", DATABASE_CONFIG.get("host", "127.0.0.1"))
DB_PORT = int(os.environ.get("DB_PORT", DATABASE_CONFIG.get("port", 5432)))
DB_USER = os.environ.get("DB_USER", DATABASE_CONFIG.get("user", "gear_user"))
DB_NAME = os.environ.get(
    "DB_NAME",
    DATABASE_CONFIG.get("dbname", DATABASE_CONFIG.get("name", "gear_app")),
)
DB_PASS = (
    os.environ.get("DB_PASSWORD")
    or os.environ.get("GEAR_DB_PASSWORD")
    or DATABASE_CONFIG.get("password")
)

TRACKING_ENABLED = _as_bool(
    os.environ.get("TRACKING_ENABLED"),
    default=_as_bool(TRACKING_CONFIG.get("enabled"), default=False),
)

TASKS_FILE_PATH = BASE_DIR / PATHS_CONFIG.get("tasks_file", "tasks.json")
...
if TRACKING_ENABLED:
    ...
    print("Tracking disabled. Set TRACKING_ENABLED=true to enable database logging.", file=sys.stderr)
...
try:
    ...
    print(f"Tasks config warning: {e}", file=sys.stderr)
...
def log_task_start():
    ...
    try:
        ...
        return jsonify({"log_id": log_id, "start_time": start_time, "tracking": True})
...
def _store_task_run(log_id: Any, trace_id: Optional[str], trace_payload: dict[str, Any]) -> None:
    if not TRACKING_ENABLED:
        return

    if log_id in (None, "", "null"):
        return
    ...
    try:
        ...
        cur.execute(
            ...
            VALUES (%s, %s, %s)
            """,
            (log_id, trace_id, json.dumps(trace_payload)),
        )
        ...
        print(f"Task run storage error: {e}", file=sys.stderr)
...
def run_orchestration():
    ...
    target_norm = (target or "crewai").strip().lower()

    if not code.strip():
        return jsonify({"error": "Empty code"}), 400

    if target_norm in ("adk", "googleadk", "google-adk"):
        code = _prepend_google_adk_imports(code)
        code_to_run = code
    else:
        code_to_run = _ensure_kickoff(code)

    instrumentation_prefix = get_instrumentation_prefix(target_norm)
    final_code = f"{instrumentation_prefix}\n{code_to_run}" if instrumentation_prefix else code_to_run
    ...
    try:
        ...
        clean_stderr = _strip_gear_trace_markers(stderr)
        ...
        return jsonify({"error": "Execution timed out after 180 seconds."}), 500
...
def log_task_end():
    ...
    try:
        ...
        return jsonify({"success": True, "duration": duration, "tracking": True})
...
def feature_model() -> Any:
    ...
    if not image_path.exists():
        return jsonify(
            {
                "error": f"Missing FeatureIDE image: {image_path.relative_to(BASE_DIR)}"
            }
        ), 500
