(function () {
  ...
  const conversionCore = window.GearConversionCore;
  ...
  const toPythonName = (value, fallback) => {
    ...
    if (/^[0-9]/.test(candidate) || pythonKeywords.has(candidate)) return `gear_${candidate}`;
    return candidate;
  };
  ...
  const assemble = (input) => {
    ...
    const outputs = {};
    const gearIR = conversionCore?.buildGearIR ? conversionCore.buildGearIR(input) : null;
  ...
  window.GearAssemblyEngine = {
    ...
    utils: {
      ...
      renderTemplate,
      getTemplate,
      buildGearIR: conversionCore?.buildGearIR,
      createConversionReport: conversionCore?.createConversionReport,
    },
  };
})();
