(function (window) {
  ...
  window.GearAssemblyPlugins["semantic-kernel"] = {
    assemble(input) {
      ...
      const helpers = ["async def _run_agent(agent: ChatCompletionAgent, prompt: str) -> str:", "    response = await agent.get_response(messages=prompt)", "    return str(response.content)"];
