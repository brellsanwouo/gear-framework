(function (window) {
  ...
  window.GearAssemblyPlugins.autogen = {
    assemble(input) {
      ...
      workflowLines.push("    return current", "", "async def main() -> None:", "    prompt = os.environ.get(\"GEAR_INPUT\", \"\")", "    try:", "        print(await run_workflow(prompt))", "    finally:", `        await asyncio.gather(${clientVars.map((client) => `${client}.close()`).join(", ")})`, "", "if __name__ == \"__main__\":", "    asyncio.run(main())");
