(function (window) {
  ...
  window.GearAssemblyPlugins["microsoft-agent-framework"] = {
    assemble(input) {
      ...
      const executorLines = [
        "async def _run_agent(agent: Agent, prompt: str):",
        "    name = getattr(agent, \"name\", agent.__class__.__name__)",
        "    return await _gear_trace_async_call(f\"agent.{name}\", prompt, lambda: agent.run(prompt), {\"gear.agent\": name})",
        "",
        ...
        "        result = str(await _run_agent(self.agent, value))",
        "        if self.output:",
        ...
        "            values = [str(value) for value in await asyncio.gather(*(_run_agent(agent, current) for agent in self.agents))]",
        "            current = \"\\n\\n\".join(values)",
        "            if self.aggregator:",
        "                current = str(await _run_agent(self.aggregator, current))",
        "        else:",
        "            for _ in range(self.turns):",
        "                for agent in self.agents:",
        "                    current = str(await _run_agent(agent, current))",
        "        if self.output:",
      ...
      graphLines.push("workflow = builder.build()", "", "async def main() -> None:", "    prompt = os.environ.get(\"GEAR_INPUT\", \"\")", "    events = await workflow.run(prompt)", "    outputs = events.get_outputs()", "    print(outputs[-1] if outputs else \"Workflow completed without output.\")", "", "if __name__ == \"__main__\":", "    asyncio.run(main())");
