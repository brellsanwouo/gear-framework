  ...
  window.GearAssemblyPlugins.crewai = {
    assemble(input) {
      ...
      ir.agents.forEach((item, index) => {
        ...
        taskLines.push(`def ${taskFactory}(context: str = "") -> Task:`, `    description = ${lit(description)}`, "    if context:", "        description += \"\\n\\nContext from the previous agent:\\n\" + context", "    return Task(", `        name=${lit(task.TaskName || `${item.name}Task`)},`, "        description=description,", `        expected_output=${lit(task.ExpectedOutput || "A useful result.")},`, `        agent=${variable},`, `        human_input=${controls.HumanInteractionControl === true ? "True" : "False"},`, "    )", "", `async def ${runner}(context: str) -> str:`, `    task = ${taskFactory}(context)`, `    crew = Crew(name=${lit(`${item.name}Crew`)}, agents=[${variable}], tasks=[task], process=Process.sequential, memory=${source.Memory === true ? "True" : "False"}, tracing=False)`, `    result = await _gear_trace_async_call(${lit(`agent.${item.name}`)}, context, lambda: crew.kickoff_async(), {"gear.agent": ${lit(item.name)}, "gen_ai.request.model": ${lit(model)}})`, "    return str(result)", "");
      ...
      const workflowLines = [`WORKFLOW_NAME = ${lit(ir.workflow.name || "GearWorkflow")}`, "", "async def run_workflow() -> str:", "    current = \"\""];
      ...
      workflowLines.push("    return current", "", "if __name__ == \"__main__\":", "    print(asyncio.run(run_workflow()))");
      const imports = ["import asyncio", "from crewai import Agent, Crew, Task, Process, LLM", "from dotenv import load_dotenv", "", "load_dotenv()"];
