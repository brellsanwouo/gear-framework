...
from gear_web.blueprints.system import create_system_blueprint
from gear_web.settings import _studio_model_policy, _studio_revision

from gear_sdk.runner import RunResult


def test_studio_revision_changes_with_served_assets(tmp_path):
    asset = tmp_path / "studio.js"
    asset.write_text("first", encoding="utf-8")
    first = _studio_revision(tmp_path)
    asset.write_text("second", encoding="utf-8")
    second = _studio_revision(tmp_path)
    assert len(first) == 12
    assert first != second


def test_version_endpoint_exposes_studio_revision():
    app = Flask(__name__)
    app.register_blueprint(create_system_blueprint({}, "revision-test"))
    payload = app.test_client().get("/api/version").get_json()
    assert payload["studio_revision"] == "revision-test"


def test_web_routes_and_build_history(tmp_path, monkeypatch):
    store_path = str(tmp_path / "web.db")
    monkeypatch.delenv("GEAR_ENABLE_LOCAL_RUNNER", raising=False)
    monkeypatch.delenv("GEAR_ENABLE_MANUAL_RUNNER", raising=False)
    web = importlib.import_module("gear_web.app")
    monkeypatch.setattr(web, "STORE_PATH", store_path)
    monkeypatch.setattr(web, "TRACKING_ENABLED", False)
    ...
    assert classic_response.status_code == 302
    assert classic_response.headers["Location"].endswith("/studio")
    classic_file_response = client.get("/ui/index.html")
    assert classic_file_response.status_code == 302
    assert classic_file_response.headers["Location"].endswith("/studio")
    ...
    assert set(studio_config["model"]) == {"locked", "provider", "model", "models"}
    assert isinstance(studio_config["model"]["locked"], bool)
    assert studio_config["model"]["provider"] == "openai"
    ...
    starter = client.get(
        ...
        query_string={"project_id": "studio-demo", "provider": "google", "model": "gpt-4o-mini"},
    ).get_json()
    assert starter["project"]["id"] == "studio-demo"
    assert len(starter["agents"]) == 5
    expected_model = studio_config["model"]["model"] if studio_config["model"]["locked"] else "gpt-4o-mini"
    assert {agent["LLMConfiguration"]["Provider"] for agent in starter["agents"]} == {"openai"}
...
"""
    ...
    monkeypatch.setattr(
        "gear_web.services.observability.record_execution",
        lambda **values: (
            "mlflow-run-1",
            "mlflow-run-1",
            {"token_usage_available": False, "cost_available": False, "llm_call_count": 0},
        ),
    )
...
def test_studio_model_policy_is_optional_and_environment_driven(monkeypatch):
    ...
    assert _studio_model_policy() == {
        ...
        "provider": "openai",
        "model": "gpt-4o-mini",
        "models": ["gpt-4o-mini", "gpt-5.4-mini", "gpt-4.1-mini"],
    }
    ...
    assert _studio_model_policy() == {
        ...
        "provider": "openai",
        "model": "claude-sonnet",
        "models": ["claude-sonnet", "gpt-4o-mini", "gpt-5.4-mini", "gpt-4.1-mini"],
    }
...
def test_studio_project_always_enforces_openai_provider():
    project = _studio_project(
        {
            "project_id": "openai-provider",
            "agents": [{
                "AgentIdentity": {"Name": "Writer", "Purpose": "Write", "ContextDescription": "Writer"},
                "LLMConfiguration": {"Provider": "google", "Model": "gpt-4o-mini"},
                "TaskSpecification": {"TaskName": "Write", "TaskDescription": "Write", "ExpectedOutput": "Text"},
            }],
            "workflow": {"WorkflowName": "Main", "Items": {"Agents": ["Writer"], "Modules": []}},
        },
        {"locked": False, "provider": "openai", "model": "gpt-5.4-mini", "models": ["gpt-5.4-mini", "gpt-4o-mini"]},
    )
    assert project.data["agents"][0]["LLMConfiguration"] == {
        "Provider": "openai",
        "Model": "gpt-4o-mini",
...
def test_studio_project_rejects_models_outside_openai_mini_list():
    payload = {
        "project_id": "unsupported-model",
        "agents": [{
            "AgentIdentity": {"Name": "Writer", "Purpose": "Write", "ContextDescription": "Writer"},
            "LLMConfiguration": {"Provider": "openai", "Model": "unsupported-model"},
            "TaskSpecification": {"TaskName": "Write", "TaskDescription": "Write", "ExpectedOutput": "Text"},
        }],
        "workflow": {"WorkflowName": "Main", "Items": {"Agents": ["Writer"], "Modules": []}},
    }
    ...
    with pytest.raises(ValueError, match="available OpenAI mini model"):
        _studio_project(payload, policy)
